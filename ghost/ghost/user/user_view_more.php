...
<body class="animsition">
	...
	<section class="sec-product-detail bg0 p-t-105 p-b-70">
					...
					if(isset($_GET['id']))
					{
						...
						while($row=mysqli_fetch_array($query))
						{
							...
							if($row['sell_status']=='Unsold')
							{
								...
								{
									$chckQuery = mysqli_query($con, "SELECT COUNT(*) FROM `listing_products` WHERE `id` = ". $_GET['id'] ." AND `user_id` = ". $_SESSION['front_user_id'] ."");
									$chckResult = mysqli_fetch_array($chckQuery);
	
									if($chckResult[0] != 0)
									{
										echo"<button disabled class='txt-s-103 btn cl0 bg10 btnaddtocart hov-btn2 trans-04 m-b-30' data-toggle='modal' data-target='#exampleModalCenter'>Talk with Person</button>";
									}
									else
									{
										echo"<button class='txt-s-103 btn cl0 bg10 btnaddtocart hov-btn2 trans-04 m-b-30' id='talkBtn'>Talk with Person</button>";
									}
								}
								else
								{
									echo"<button class='txt-s-103 btn cl0 bg10 btnaddtocart hov-btn2 trans-04 m-b-30' id='sendToLogin'>Talk with Person</button>";
								}
		...
		<div class="tab02 p-t-80 tab_detail">
				<!-- Nav tabs -->
				...
				<div class="tab-content">
					...
					<div class="tab-pane fade show active" id="description" role="tabpanel">
						<?php
							$customQuery = mysqli_query($con, "SELECT `user_master`.`owner_name`, `user_master`.`email`, `user_master`.`address`, `user_master`.`phone` FROM `user_master` JOIN `listing_products` ON `listing_products`.`user_id` = `user_master`.`id` WHERE `listing_products`.`id` = ". $_GET['id'] .";");
					...
					<div class="tab-pane fade" id="info" role="tabpanel">
						<h5 class="m-5 cl9 size-w-55">
							There is no return order policy available for customer products.
						</h5>
						<!-- <ul class="p-t-21">
							<li class="txt-s-101 flex-w how-bor2 p-tb-14">
								<span class="cl6 size-w-54">
			...
			<div class="wrap-slick9">
				...
				<?php
					if(isset($_GET['id']))
					{
							...
							$query_record=mysqli_query($con,"select * from listing_products where catagory_id=".$ans['catagory_id']." and id!=".$_GET['id']." and sell_status='Unsold' and product_status='Active' LIMIT 4");
							while($row=mysqli_fetch_array($query_record))
							{
								...
								// echo"<a href='#' class='txt-m-103 cl3 txt-center chkViewMore related_name hov-cl10 trans-04 js-name-b1' data-id='".$row['id']."'>".$row['product_name']."</a>";
